...
import { createUserToken, passwordDigest, userCookie } from '../../lib/user-session.js';
...
export default async function handler(req, res) {
  ...
  const account = await Account.findOne({ email }).select('+passwordHash +passwordSalt');
  ...
  return res.json({ accountId: String(account._id), name: account.name, email: account.email, roles: account.roles, role: requestedRole === 'salon_owner' ? 'owner' : 'customer', provider: account.provider });
}
