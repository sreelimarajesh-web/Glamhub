...
import { createUserToken, newPassword, userCookie } from '../../lib/user-session.js';
...
export default async function handler(req, res) {
  ...
  try {
    const account = await Account.create({ name: String(name).trim(), email: normalizedEmail, roles: [role], ...newPassword(password) });
    res.setHeader('Set-Cookie', userCookie(createUserToken(account)));
    return res.status(201).json({ accountId: String(account._id), name: account.name, email: account.email, roles: account.roles, role: role === 'salon_owner' ? 'owner' : 'customer', provider: account.provider });
  } catch (error) {
