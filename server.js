...
app.get('/api/health', async (_req, res) => { const database = await mongodbHealth(); res.status(database.ok ? 200 : 503).json({ ok: database.ok, app: 'Zaya', database: 'mongodb', ...database }); });
