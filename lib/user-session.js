...
export function createUserToken(account, activeRole) {
  const role = ['customer', 'salon_owner'].includes(activeRole) ? activeRole : undefined;
  const value = encode({ sub: String(account._id), email: account.email, ownerName: account.ownerName || account.name, avatarUrl: account.avatarUrl || null, roles: account.roles, ...(role ? { activeRole: role } : {}), exp: Date.now() + 8 * 60 * 60 * 1000 });
  return `${value}.${signature(value, sessionSecret())}`;
}
...
export function readUserSession(cookieHeader = '') {
  try {
    ...
    const expected = signature(value, sessionSecret());
