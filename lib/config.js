...
export function validateProductionConfig() { if (process.env.NODE_ENV !== 'production') return; sessionSecret(); adminConfig(); publicAppUrl(); if (!value('MONGODB_URI')) throw new ConfigurationError('Database is not configured.'); if (!rateLimitStoreConfig()) throw new ConfigurationError('A shared rate-limit store is required.'); if (value('GOOGLE_AUTH_ENABLED') === 'true' && !value('GOOGLE_OAUTH_CLIENT_ID')) throw new ConfigurationError('Google authentication is not configured.'); }
