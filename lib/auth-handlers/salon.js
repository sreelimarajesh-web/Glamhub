...
export default async function handler(req, res) {
  ...
  const activeAccount = await Account.findById(actor.sub).select('status roles').lean();
  if (!activeAccount || activeAccount.status !== 'active' || !activeAccount.roles.includes('salon_owner')) return res.status(403).json({ error: 'Salon owner access required.' });
  if (req.method === 'GET') {
  ...
  const hasLatitude = 'latitude' in (req.body || {});
  const hasLongitude = 'longitude' in (req.body || {});
  if (hasLatitude !== hasLongitude) return res.status(400).json({ error: 'Both latitude and longitude are required.' });
  if (hasLatitude) {
    const latitude = Number(req.body.latitude);
    const longitude = Number(req.body.longitude);
    if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90 || !Number.isFinite(longitude) || longitude < -180 || longitude > 180) return res.status(400).json({ error: 'Choose a valid location on the map.' });
